...
import { GAME_START_EVENT, GameBoard, PLAY_CARD_EVENT, TURN_END_EVENT } from "./game_board";
...
export class Duel extends Scene {
  ...
  override onPreUpdate(engine: Engine, elapsedMs: number): void {
    ...
    const newHandCards = this._gameBoard.playerHand.filter((c) => !this._playerHand.has(c.id));
    newHandCards.forEach((card, index) => {
            const cardPos = vec(PLAYER_HAND_START_X + (CARD_WIDTH * index + numCardsInHand + SPACE_BETWEEN_CARDS * index + numCardsInHand), PLAYER_HAND_START_Y)
    ...
    const newBoardCards = this._gameBoard.playerBoard.filter((c) => !this._playerBoard.has(c.id));
    newBoardCards.forEach((card, index) => {
        const cardPos = vec(PLAYER_BOARD_START_X + (CARD_WIDTH * index + numCardsOnBoard + SPACE_BETWEEN_CARDS * index + numCardsOnBoard), PLAYER_BOARD_START_Y);
